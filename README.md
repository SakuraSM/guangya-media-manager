...
- 同一作品存在多个版本时，按质量优先、兼容优先或节省空间规则自动选取，可配置每组保留 1–3 个或全部版本。
- 可选在整理成功后将源媒体、关联字幕和明确过滤的无关文件移入光鸭回收站；默认关闭，不自动清空回收站。
- 持续整理规则支持手动、固定间隔和 Cron 调度，通过目录快照只处理新增或变化文件。
- 规则可在创建后立即首扫，安全自动流支持高置信审批、无阻断自动整理和失败退避重试。
- API 与 Worker 共享云盘请求闸门，读写错峰、写操作降速，命中频控后自动退避。
...
应用默认会限制光鸭 API 的调用节奏：读取请求至少间隔 `0.35` 秒，写入请求至少间隔 `1.2` 秒，后台任务轮询至少间隔 `1` 秒，并加入少量随机抖动。API 与 Worker 通过 Redis 共用同一个请求窗口，避免扫描、目录浏览和整理同时运行时形成突发请求。

收到明确的频控响应后，系统会指数退避再重试。复制、移动、重命名和上传等写操作遇到普通网络超时不会自动重放，以免产生重复文件。账号仍频繁触发限制时，可在 `.env` 中调大以下参数：

```dotenv
GUANGYA_API_READ_INTERVAL_SECONDS=0.5
GUANGYA_API_WRITE_INTERVAL_SECONDS=2
GUANGYA_API_POLL_INTERVAL_SECONDS=1.5
GUANGYA_API_BACKOFF_MAX_SECONDS=60
```

调整后执行 `docker compose restart api worker`。不建议把间隔设为 `0`，也不要同时启动多个独立部署连接同一账号。
...
- 同名但内容不同的目标文件不会被覆盖。
- 失败或取消的任务不会自动删除暂存文件。
- 源文件清理默认关闭；启用后仅在成功发布后移入光鸭回收站，系统不会自动清空回收站。
